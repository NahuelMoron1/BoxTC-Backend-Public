...
import { Drivers, GuessDrivers, GuessDrivers_Teams, Teams } from "../models/mysql/associations";
...
export const getGameOfTheDay = async (req: Request, res: Response) => {
  try {
    ...
    const game = await GuessDrivers.findOne({
      where: {
        date: today,
      },
      include: [
      ...
      order: [[GuessDrivers_Teams, 'ordered', 'ASC']],
    });
    ...
    const gameTeams = game.get("GuessDrivers_Teams") as any[];
    ...
    const transformedTeams = gameTeams.map(team => {
      return {
        ...
        Team: {
          id: team.team?.id || '',
          name: team.team?.name || '',
          common_name: team.team?.common_name || '',
          logo: team.team?.logo || ''
        },
...
export const getNextTeam = async (req: Request, res: Response) => {
  try {
    ...
    const teams = game.get("GuessDrivers_Teams") as any[];
